...
#         if (len(batch) % cfg.num_device) != 0:
#             batch = batch[:-(len(batch) % cfg.num_device)]
...
#             all_batches.append(batch)
    ...
    def get_eval_data(self, set_name='dev'):
        ...
        self.set_stats[set_name]['num_turn'] = num_turns
        self.set_stats[set_name]['num_dial'] = num_dials
    ...
    def get_batches(self, set_name):
        ...
        if cfg.use_low_resource and set_name == 'train':
            dial = random.sample(dial, cfg.num_low_resource)
        ...
        for k in turn_bucket:
            ...
            num_turns += k * len(batches) * cfg.batch_size
            num_dials += len(batches) * cfg.batch_size
            all_batches += batches
...
class MultiWozReader(_ReaderBase):
    def __init__(self, tokenizer):
        ...
        self.domain_files = json.loads(open(cfg.path_domain_files, 'r').read())
        self.slot_value_set = json.loads(
            open(cfg.path_slot_value_set_processed, 'r').read())
        if cfg.use_multi_act_train:
            self.multi_acts = json.loads(open(cfg.path_multi_act_train, 'r').read())

        test_list = [l.strip().lower()
                     for l in open(cfg.path_lst_test, 'r').readlines()]
        dev_list = [l.strip().lower()
                    for l in open(cfg.path_lst_val, 'r').readlines()]
    ...
    def _build_vocab(self):
        ...
        vp = cfg.path_vocab_train if cfg.mode == 'train' or cfg.path_vocab_eval is None else cfg.path_vocab_eval
    ...
    def _load_data(self, save_temp=True):
        ...
        if save_temp: # save encoded data
            if 'all' in cfg.exp_domains:
                encoded_file = os.path.join(cfg.dir_data_processed, 'new_db_se_blank_encoded.data.json') 
            ...
            if os.path.exists(encoded_file):
                ...
                self.data = json.loads(
                    open(cfg.dir_data_processed+cfg.file_data, 'r', encoding='utf-8').read().lower())
...
#                         continue
    ...
    def convert_batch(self, py_batch, py_prev, first_turn=False):
        ...
        if cfg.use_multi_act_train and cfg.mode == 'train':
