...
class MultiWozEvaluator(object):
    ...
    ##### 2022/1/6 #####
    def unpack_dial(self, dials):
        data = []
        for dial_id in dials:
            data.append(dials[dial_id])
        return data
    ####################
    ...
    ##### 2022/1/6 #####
    def validation_metric_results(self, data):
        bleu = self.bleu_metric(data)
        # accu_single_dom, accu_multi_dom, multi_dom_num = self.domain_eval(data)
        success, match, req_offer_counts, dial_num, data = self.context_to_response_eval_results(data,
                                                                                        same_eval_as_cambridge=cfg.same_eval_as_cambridge)
        return bleu, success, match, data
    ####################
    ...
    ##### 2022/1/6 #####
    def context_to_response_eval_results(self, data, eval_dial_list = None, same_eval_as_cambridge=False):
        dials = self.pack_dial(data) # dict of dialog list
        counts = {}
        for req in self.requestables:
            counts[req+'_total'] = 0
            counts[req+'_offer'] = 0
        ...
        for dial_id in dials:
            if eval_dial_list and dial_id +'.json' not in eval_dial_list:
                continue
            dial = dials[dial_id] # list of turn dict
            reqs = {}
            goal = {}
            if '.json' not in dial_id and '.json' in list(self.all_data.keys())[0]:
                dial_id = dial_id + '.json'
            ...
            # success, match, stats, counts = self._evaluateGeneratedDialogue(dial, goal, reqs, counts,
            #                                                         same_eval_as_cambridge=same_eval_as_cambridge)
            success, match, stats, counts, dial = self._evaluateGeneratedDialogue_results(dial, goal, reqs, counts,
                                                                    same_eval_as_cambridge=same_eval_as_cambridge)
            
            # Add 'match', 'success' to every turns in dial
            dials[dial_id] = dial
        ...
        data = self.unpack_dial(dials)
        return succ_rate, match_rate, counts, dial_num, data
    ####################
    ...
    ##### 2022/1/6 #####
    def _evaluateGeneratedDialogue_results(self, dialog, goal, real_requestables, counts,
                                                          soft_acc=False, same_eval_as_cambridge=False):
        """Evaluates the dialogue created by the model.
            First we load the user goal of the dialogue, then for each turn
            generated by the system we look for key-words.
        ...
        # add 'match', 'success' to dialog
        dialog[0]['match'] = match 
        dialog[0]['success'] = success
        return success, match, stats, counts, dialog
    ####################
