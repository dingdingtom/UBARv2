...
class Evaluator:
    ...
    def __init__(self, bleu : bool, success : bool, richness : bool, output_path : str, dst : bool = False):
        self.bleu = bleu
    ...
    def evaluate(self, input_data):
        ...
        return {
            ...
            "success"  : get_success(input_data, self.database, self.goals, self.booked_domains, self.output_path)  if self.success else None,
...
def get_success(input_data, database, goals, booked_domains, output_path):
    ...
    ##### 2022/3/12 使用"dials_match_success"字典保存每个对话的match和success #####
    dials_match_success = {}
    #################################################
    ...
    for dialog_id, dialog in input_data.items(): 
        ...
        ##### 2022/3/12 记录每个对话的match和success #####
        if match['total']:
            dials_match_success[dialog_id] = {"match": 1, "success": 1 if success['total'] else 0}
        else: # 如果 match 为0
            dials_match_success[dialog_id] = {"match": 0, "success": 0}
        #################################################
    ...
    ##### 2022/3/12 将"dials_match_success"字典保存为json文件#####
    json.dump(dials_match_success, open(os.path.join(output_path, "MATCH_SUCCESS.json"), 'w'), indent=2)
    #############################################################
