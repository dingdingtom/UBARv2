...
class _ReaderBase(object):
    ...
    def _bucket_by_turn(self, encoded_data):
        ...
        for dial in encoded_data:
            turn_len = len(dial)    # len(dial)表示一个对话会话的轮数
        ...
        # for k in del_l:
        #    turn_bucket.pop(k)
        return OrderedDict(sorted(turn_bucket.items(), key=lambda i: i[0])) # OrderedDict元素value如何排序
    ...
    def get_eval_data(self, set_name='dev'):
        ...
        if cfg.multi_gpu:
            ...
            len_dev = len(self.dev)
            name_to_set = {'train': self.train[cfg.local_rank: len_train: cfg.world_size], 'test': self.test[cfg.local_rank: len_test: cfg.world_size], 'dev': self.dev[cfg.local_rank: len_dev: cfg.world_size]}
        else:
    ...
    def get_batches(self, set_name):
        ...
        name_to_set = {'train': self.train, 'test': self.test, 'dev': self.dev}
        dial = name_to_set[set_name]
        if cfg.low_resource and set_name == 'train': # low resource setting
        ...
        for k in turn_bucket:
            ...
            logging.info("Get data batch in {} level".format(cfg.context_scheme))
            if cfg.context_scheme == 'URURU' or cfg.context_scheme == 'UBARU_GEN_CONTEXT':
                num_training_steps_per_epoch += k * len(batches)
            if cfg.context_scheme == 'UBARU':
                num_training_steps_per_epoch += len(batches)
        ...
        if cfg.multi_gpu:
            self.set_stats[set_name]['num_training_steps_per_epoch'] = num_training_steps_per_epoch // (cfg.world_size*cfg.gradient_accumulation_steps)
        else:
            self.set_stats[set_name]['num_training_steps_per_epoch'] = num_training_steps_per_epoch // (cfg.gradient_accumulation_steps)
        self.set_stats[set_name]['num_turns'] = num_turns
        self.set_stats[set_name]['num_dials'] = num_dials
    ...
    def get_nontranspose_data_iterator(self, all_batches):
        # (2021/12/20)
        if cfg.multi_gpu:
            batch_len = len(all_batches)
            all_batches = all_batches[cfg.local_rank: batch_len: cfg.world_size]
...
class MultiWozReader(_ReaderBase):
    def __init__(self, tokenizer):
        ...
        # 为什么只在'train'时添加 (2021/12/19)，因为推理时已经加载过tokenizer了
        if cfg.mode=='train':
    ...
    ##### 2021/12/31 ######
    def bspan_to_DBpointer_batch(self, bspan_batch, turn_domain_batch):
        vector_batch = []
        for bspan, turn_domain in zip(bspan_batch, turn_domain_batch):
            constraint_dict = self.bspan_to_constraint_dict(bspan)
            # print(constraint_dict)
            matnums = self.db.get_match_num(constraint_dict)
            match_dom = turn_domain[0] if len(turn_domain) == 1 else turn_domain[1]
    ...
    ##### 2021/12/28 #####
    def convert_turn_eval_batch(self, turn_batch, pv_turn_batch, first_turn=False):
        inputs = {}
        inputs['context'] = []
        inputs['labels'] = []
        for batch_idx in range(len(turn_batch['dial_id'])):
            context_list = []
            # predict_list = []
            prompt = ''
            if cfg.use_true_curr_bspn:
            ...
            if first_turn:
                context = []
                for c in context_list:
                    context += turn_batch[c][batch_idx]

                inputs['context'].append(context + self.tokenizer.encode([prompt]))
                inputs['labels'].append(context)
                # e43 with BABAU
                # inputs['labels'] = []
                ...
                pv_context = None
                if cfg.use_true_curr_bspn:
                    if cfg.use_true_curr_aspn is not True:
                        pv_context = pv_turn_batch['labels'][batch_idx] + pv_turn_batch['aspn'][batch_idx] + pv_turn_batch['resp'][batch_idx]
                
                else:
                    pv_context = pv_turn_batch['labels'][batch_idx] + pv_turn_batch['bspn'][batch_idx] + pv_turn_batch['db'][batch_idx] + pv_turn_batch['aspn'][batch_idx] + pv_turn_batch['resp'][batch_idx]
                # e43 with BABAU
                # pv_context = pv_turn['labels'] + pv_turn['bspn'] + pv_turn['db'] + pv_turn['aspn']
                    
                # prompt response, add sos_r
                inputs['context'].append(pv_context + context + self.tokenizer.encode([prompt]))
                # context just the current turn
                # inputs['context'] = context + self.tokenizer.encode([prompt])
                # context just the current action

                if cfg.use_all_previous_context:
                    inputs['labels'].append(pv_context + context) # use all previous ubar history
                else:
                    inputs['labels'].append(context) # use privosu trun

            if len(inputs['context'][batch_idx]) > 900:
                # print('len exceeds 900')
                inputs['context'][batch_idx] = inputs['context'][batch_idx][-900:]
...
########## add 'convert_batch_turn_gen' (2021/12/19)##########
    def convert_batch_turn_gen(self, turn_batch, pv_batch, first_turn=False, gen_context=cfg.gen_context):
        """
        UBARU
        convert the current and all the previous turn using generated response
        concat [U_0, B_0, DB_0, A_0, R_0_hat, ..., U_{t-1}, B_{t-1}, DB_{t-1}, A_{t-1}, R_{t-1}_hat, U_t, B_t, DB_t, A_t, R_t]
        firts turn: [U_t, B_t, DB_t, A_t, R_t]
        try: [user, bspn, db, aspn, resp]
        ...
        if first_turn:
            ...
            batch_zipped = zip(pv_batch,
                               turn_batch['user'], turn_batch['bspn'], turn_batch['db'], turn_batch['aspn'], turn_batch['resp'])
            for pv, u, b, db, a, r in batch_zipped:
                context = pv + u + b + db + a + r
                contexts.append(context)
                label = None
                if gen_context == 'r':
                    label = pv + u + b + db + a
